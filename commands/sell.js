	...
	async execute(interaction) {
		...
		if (!integer)
			return await interaction.reply({
				content: `<@${interaction.user.id}>, Вы **не** указали **цену** за которую хотите **продать** роль`,
				ephemeral: true
			})
		if (integer < 50)
			return await interaction.reply({
		...
		const myItems = await db.get('myItems')
		if (!myItems) {
		...
		if (interaction.member._roles.find(rolea => rolea == role.id)) {
			...
			collector.on('collect', async i => {
				...
				if (i.customId === 'yes') {
					...
					await db.push('myItems', { id: interaction.user.id, role: role, match: integer })
