...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('roll')
		.setDescription('Ежедневная награда на сегодня.'),
	async execute(interaction) {
		...
		const embed = new MessageEmbed()
			...
			.setTitle('Временные награды')
			.setThumbnail(`${interaction.user.displayAvatarURL({ dynamic: true })}`)
			.setDescription(`<@${interaction.user.id}> Вы **уже** забрали **временную** награду! Вы можете **получить** следующую через **${hours}** часа, **${minutes}** минут, **${seconds}** секунду`, true)
		return interaction.reply({
			"embeds": [embed],
		});
	},
